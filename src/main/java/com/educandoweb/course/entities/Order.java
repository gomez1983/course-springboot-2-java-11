...
public class Order implements Serializable {
	...
	@OneToOne(mappedBy = "order", cascade = CascadeType.ALL) // Significa que este atributo "payment" tem dependência de 1 por 1 em "order". As duas entidades terão o mesmo código Id. Por isso, o CascadeType é obrigatório entrar;
	...
	public Double getTotal() {
		double sum = 0.0; // Soma começa com 0
		for (OrderItem x : items) { // Faz a varredura de todos os items. Pra todo item de pedido...
			sum = sum + x.getSubTotal(); // a soma recebe ela mesma + o subtotal de cada item de pedido.
		}
		return sum;
	}
